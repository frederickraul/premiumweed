...
[
  ...
  const ChatBox = (
    ...
    <div className="sticky flex items-center justify-between border-b border-stroke px-6 py-4.5 dark:border-strokedark">
      <div className="flex items-center">
        <div className="mr-4.5 h-13 w-full max-w-13 overflow-hidden rounded-full">
          <img src={activeReceiver.image || defaultImage} alt="avatar" className="h-full w-full object-cover object-center" />
        </div>
        <div>
    ...
    <div className="no-scrollbar max-h-full space-y-3.5 overflow-auto px-6 py-7.5">
      {
        activeMessages?.length < 1 && <NoMessages/>
 
      }
      { 
      // activeMessages?.map((message:any, key:number)=>(
      //   <div className={`
      //       max-w-125
      //       ${message.senderId == props.currentUser?.id && 'ml-auto'}
      //       `}>
      //     <p className={`
      //         mb-2.5
      //         text-sm
