...
const ChatBox = (props:{
    ...
    useEffect(()=>{
      // setMessageList(props.activeMessages)
      console.log(props.activeMessages);
        waitTime();
  ...
  return (
    ...
    <div className="sticky flex items-center justify-between border-b border-stroke px-6 py-4.5 dark:border-strokedark">
      <div className="flex items-center">
        <div className="mr-4.5 h-13 w-full max-w-13 overflow-hidden rounded-full">
          <img src={props?.activeReceiver?.image || defaultImage} alt="avatar" className="h-full w-full object-cover object-center" />
        </div>
        <div>
          <h5 className="font-medium text-black dark:text-white">
            {props?.activeReceiver?.name || "Not available"}
          </h5>
    ...
    <div className="no-scrollbar max-h-full space-y-3.5 overflow-auto px-6 py-7.5">
      {
        (!props.activeMessages || props.activeMessages?.length < 1) && <NoMessages/>
 
      }

{ props.activeMessages?.map((message:any, key:number)=>(
        <div key={key} className={`
            max-w-[350px]
            flex
            mb-5
            ${message.senderId == props.currentUser?.id && 'ml-auto justify-end'}
            `}>
             <div className="mr-4.5 h-13 w-full max-w-13 overflow-hidden rounded-full">

             { message.senderId != props.currentUser?.id &&
                <img src={props?.activeReceiver?.image || defaultImage} 
                  alt="avatar" 
                  className="h-full 
                             w-full 
                             object-cover 
        ...
        <div>
          ...
          <p className={`
              ...
              text-sm
              font-medium
             ${message.senderId == props.currentUser?.id && 'hidden'}
            `}>
              {props?.activeReceiver?.name}
          </p>
          ...
          <div className={`
              mb-2.5 
              rounded-2xl 
              ${message.senderId == props.currentUser?.id ?'rounded-br-none ' : 'rounded-tl-none'}
              ${message.senderId == props.currentUser?.id ?'bg-primary' : 'bg-gray dark:bg-boxdark-2'}
              px-5 
              py-3 
              `}>
            <p className={`
              font-medium
              ${message.senderId == props.currentUser?.id && 'text-white'}
              `}>
             {message?.content}
            </p>
          </div>
          <p className={`
              text-xs 
              font-medium
              ${message.senderId == props.currentUser?.id && 'text-right'}
              `}>
                1:55pm
            </p>
            </div>
      ...
      <div className="flex items-center justify-between space-x-4.5">
        <div className="relative w-full">
          ...
          value={value}
            ...
            className="h-13 w-full rounded-md border 
            border-stroke 
            bg-gray 
            pl-5 
            pr-19 
        ...
        <button onClick={onSendMessage} className="flex h-13 w-full max-w-13 items-center justify-center rounded-md bg-primary text-white hover:bg-opacity-90">
